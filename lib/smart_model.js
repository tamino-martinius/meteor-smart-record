...
SmartModel = class SmartModel {
  ...
  static getSchema(withTimestamps = false) {
    ...
    if (withTimestamps === true) {
      schema.createdAt = {type: Date, optional: true};
      schema.updatedAt = {type: Date, optional: true};
    }
  ...
  static collection() {
    ...
    if (!collection) {
      ...
      collection.attachSchema(this.simpleSchema(true));
    }
    return collection;
  }
  ...
  destroy(skipCallbacks) {
    ...
    for (const relationName of this.constructor._getHasOneDependentDestroy()) {
      if (_.isObject(this[relationName])) {
        this[relationName].destroy();
      }
    }
    for (const relationName of this.constructor._getHasManyDependentDestroy()) {
